...
public class CommentedThirdPersonController : MonoBehaviour
{
    ...
    [Tooltip("This value is added to the _speed value while the character is sprinting.")]
    ...
    void Update()
    {
        ...
        if ( cc.isGrounded )
        {
            ...
            // Check the player's _speed and whether it is high enough to trigger the running animation
    ...
    private void FixedUpdate()
    {
        ...
        // Checks if the player is sprinting, because if he is, it will add a _speed boost to his movement
        float velocityAdittion = 0;
        if ( isSprinting )
        {
            velocityAdittion = sprintAdittion;
        }
        // Checks if the player is couching, because if he is, it will add a _speed debuff
        if (isCrouching)
        {
        ...
        // And if it moved, let's make it faster by multiplying by _speed and reducing by Time.DeltaTime
...
the _speed could never be set to 5 or 10. We would have to use values like 0.015f or 0.0087f
